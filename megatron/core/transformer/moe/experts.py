...
class GroupedMLP(MegatronModule):
    ...
    def __init__(
        ...
        if config.use_cpu_initialization:
            assert not self.config.moe_split_expert_weights
            ...
            if self.config.moe_use_custom_function and self.config.moe_split_expert_weights:
                self.weight1 = []
                self.weight2 = []
                for i in range(self.num_local_experts):
                    ...
                    if config.perform_initialization:
                        _initialize_affine_weight_gpu(
                            self.weight1[i], config.init_method, partition_dim=1, is_expert=True
                        )
                        _initialize_affine_weight_gpu(
                            self.weight2[i], config.output_layer_init_method, partition_dim=0, is_expert=True
                        )
        ...
        if config.moe_use_custom_function and config.delay_wgrad_compute:
            ...
            if self.config.moe_split_expert_weights:
                for i in range(self.num_local_experts):
    ...
    def forward(
        ...
        if self.config.moe_use_custom_function:
            assert self.config.moe_split_expert_weights
